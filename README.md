...
Benchmarks run on Vast.ai (1x RTX 3060 12GB, driver 560.35.03, CUDA 12.4, AMD EPYC 7402 24-Core, 251 GiB RAM, Ubuntu 22.04).

| Backend | Single hash time | Batch time (4096 pairs) | Speedup vs CPU (single) | Speedup vs CPU (batch) |
| --- | --- | --- | --- | --- |
| CPU (`Pedersen`) | 27.26 µs | 35.13 ms | 1.00x | 1.00x |
| GPU (`PedersenGpu`, CUDA) | 4.68 ms | 381.59 ms | 0.006x | 0.09x |

Notes:
* Bench command: `cargo bench --bench storage --features "bench pedersen-gpu-cuda" -- pedersen` (CPU: `--features "bench"`).
* Batch benchmark uses 4096 pairs and `sample_size=10` to keep runtime manageable.
* GPU activity observed during the batch run (3-4% utilization, ~85 MiB memory via `nvidia-smi`).
